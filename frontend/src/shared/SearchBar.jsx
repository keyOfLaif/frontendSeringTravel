...
const SearchBar = () => {
    ...
    const searchHandler = async()=>{
        ...
        const res = await fetch(`${BASE_URL}/trip/search/getTripBySearch?city=${location}&distance=${distance}&maxGroupSize=${maxGroupSize}`)

        if(!res.ok) alert('Something went wrong')

        const result = await res.json()

        navigate(`/trips/search?city=${location}&distance=${distance}&maxGroupSize=${maxGroupSize}`,
        {state: result.data})
    }
